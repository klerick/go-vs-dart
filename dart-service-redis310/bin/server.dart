...
void main() async {
  ...
  redisCmd = await redisConn.connect(redisHost, redisPort);
...
Future<void> _handleCreateOrder(HttpRequest req) async {
  ...
  if (userId is! int || productId is! int || quantity is! int) {
    _sendJson(req.response, 400, {'error': 'user_id, product_id, and quantity are required integers'});
    return;
  }
  if (quantity <= 0) {
  ...
  final userRaw = await redisCmd.send_object(['GET', 'user:$userId']);
  if (userRaw == null) {
  ...
  await redisCmd.send_object(['DEL', 'order_cache:$userId']);
